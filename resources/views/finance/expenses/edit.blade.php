            ...
            <form method="POST" action="{{ route('expenses.update', $expense) }}">
                ...
                <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <select id="category" name="category" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->value }}" @selected(old('category', $expense->category->value) === $cat->value)>{{ $cat->label() }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="amount" class="block mt-1 w-full" type="number" step="1" name="amount" :value="old('amount', $expense->amount)" required min="1" />
