<x-app-layout>
    <x-slot name="header">Penjualan Hewan</x-slot>

    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <p class="text-sm text-gray-500">Catatan penjualan hewan ternak dari kandang.</p>
            @can('create-livestock-sale')
                <a href="{{ route('livestock-sales.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors">
        ...
        <form method="GET" class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="flex flex-wrap items-end gap-3">
                ...
                @if(request()->hasAny(['date', 'cage_id']))
                    <a href="{{ route('livestock-sales.index') }}" class="text-sm text-gray-500 hover:text-gray-700 h-10 flex items-center">Reset</a>
                @endif
            </div>
        </form>
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            @if($sales->isEmpty())
                <div class="px-5 py-12 text-center text-gray-500"><p class="text-sm">Belum ada data penjualan hewan.</p></div>
            @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-5 py-3 text-right text-xs font-medium text-gray-500 uppercase">Harga/ekor</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($sales as $sale)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-5 py-3 text-sm text-gray-700">{{ $sale->cage->name }}</td>
                                    <td class="px-5 py-3 text-sm text-gray-900 text-right font-semibold">{{ number_format($sale->quantity) }} ekor</td>
                                    <td class="px-5 py-3 text-sm text-gray-700 text-right">Rp {{ number_format($sale->price_per_unit) }}</td>
                                    ...
                                    <td class="px-5 py-3 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            @can('create-livestock-sale')
                                                <a href="{{ route('livestock-sales.edit', $sale) }}" class="text-sm text-blue-600 hover:text-blue-700">Edit</a>
                                                <form method="POST" action="{{ route('livestock-sales.destroy', $sale) }}" onsubmit="return confirm('Yakin hapus data ini?')">
                                                    @csrf @method('DELETE')
