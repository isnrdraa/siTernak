<x-app-layout>
    <x-slot name="header">Edit Konsumsi Pakan</x-slot>

    <div class="max-w-2xl">
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <form method="POST" action="{{ route('feed-logs.update', $feedLog) }}">
                @csrf
                @method('PUT')
                <div class="space-y-5">
                    <div>
                        ...
                        <select id="cage_id" name="cage_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            @foreach($cages as $cage)
                                <option value="{{ $cage->id }}" @selected(old('cage_id', $feedLog->cage_id) == $cage->id)>{{ $cage->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="date" class="block mt-1 w-full" type="date" name="date" :value="old('date', $feedLog->date->format('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <select id="feed_stock_id" name="feed_stock_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($feedStocks as $fs)
                                    <option value="{{ $fs->id }}" @selected(old('feed_stock_id', $feedLog->feed_stock_id) == $fs->id)>{{ $fs->feed_type }} ({{ number_format($fs->current_stock_kg, 1) }} kg)</option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <x-text-input id="feed_type" class="block mt-1 w-full" type="text" name="feed_type" :value="old('feed_type', $feedLog->feed_type)" required />
                            <x-input-error :messages="$errors->get('feed_type')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="quantity_kg" value="Jumlah (kg)" />
                            <x-text-input id="quantity_kg" class="block mt-1 w-full" type="number" step="0.01" name="quantity_kg" :value="old('quantity_kg', $feedLog->quantity_kg)" required min="0.01" />
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="2" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $feedLog->notes) }}</textarea>
